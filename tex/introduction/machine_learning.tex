...
However, plotting the values of the data sets reveals that each one of those data sets obeys a different, unique characteristic (see Figure~\ref{fig:ancombe}).

\begin{figure}
\centering
\includegraphics[width=0.9\textwidth,valign=t]{tex/introduction/ancombe.png}
\caption{
Plots of the four data sets of Ancombe's quartet \todo{cite wikipedia}. All data sets have the same mean and sample variance for both axes, the same correlation between the axes, and the same regression lines.
}
\label{figs:ancombe}
\end{figure}
